...
const (
	PROXY_REALM    = "w3securitycli_realm"
	PROXY_USERNAME = "w3securitycli"
)
...
func NewWrapperProxy(insecureSkipVerify bool, cacheDirectory string, cliVersion string, debugLogger *log.Logger) (*WrapperProxy, error) {
	...
	certName := "w3security-embedded-proxy"
	...
	certFile, err := os.CreateTemp(tmpDirectory, "w3security-cli-cert-*.crt")
	if err != nil {
	...
	if extraCaCertFile, ok := os.LookupEnv(constants.W3SECURITY_CA_CERTIFICATE_LOCATION_ENV); ok {
...
func (p *WrapperProxy) replaceVersionHandler(r *http.Request, ctx *goproxy.ProxyCtx) (*http.Request, *http.Response) {
	// Manipulate Header (replace x-w3security-cli-version)
	existingValue := r.Header.Get("x-w3security-cli-version")
	if existingValue != "" {
		p.DebugLogger.Printf("Replacing value of existing x-w3security-cli-version header (%s) with %s\n", existingValue, p.cliVersion)
		r.Header.Set("x-w3security-cli-version", p.cliVersion)
	}
	return r, nil
}
