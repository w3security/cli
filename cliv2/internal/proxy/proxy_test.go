...
func Test_canGoThroughProxy(t *testing.T) {
	...
	res, err := proxiedClient.Get("https://static.w3security.io/cli/latest/version")
	if err != nil {
		t.Fatal(err)
	}
...
func Test_xSnykCliVersionHeaderIsReplaced(t *testing.T) {
	...
	ts := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		capturedVersion = r.Header.Get("x-w3security-cli-version")
	}))
	defer ts.Close()
	...
	// request without the "x-w3security-cli-version" header set
	...
	req.Header.Add("x-w3security-cli-version", "1.0.0")
	res, err = proxiedClient.Do(req)
	if err != nil {
		t.Fatal(err)
	}
...
func Test_SetUpstreamProxy(t *testing.T) {
	...
	testUrl, _ := url.Parse("http://www.w3security.io")
...
func Test_appendExtraCaCert(t *testing.T) {
	...
	os.Setenv(constants.W3SECURITY_CA_CERTIFICATE_LOCATION_ENV, file.Name())
	...
	// cleanup
	os.Unsetenv(constants.W3SECURITY_CA_CERTIFICATE_LOCATION_ENV)
	os.Remove(file.Name())
}
