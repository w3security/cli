...
import (
	...
	"github.com/w3security/cli/cliv2/internal/cliv2"
...
func getCacheDir() string {
	cacheDir := path.Join(os.TempDir(), "w3security")
...
func Test_PrepareV1EnvironmentVariables_Fill_and_Filter(t *testing.T) {
	...
	expected := []string{"something=1",
		"in=2",
		"here=3=2",
		"W3SECURITY_INTEGRATION_NAME=foo",
		"W3SECURITY_INTEGRATION_VERSION=bar",
		"HTTP_PROXY=proxy",
		"HTTPS_PROXY=proxy",
		"NODE_EXTRA_CA_CERTS=cacertlocation",
		"W3SECURITY_SYSTEM_NO_PROXY=noProxy",
		"W3SECURITY_SYSTEM_HTTP_PROXY=httpProxy",
		"W3SECURITY_SYSTEM_HTTPS_PROXY=httpsProxy",
		"NO_PROXY=" + constants.W3SECURITY_INTERNAL_NO_PROXY + ",noProxy",
	}
...
func Test_PrepareV1EnvironmentVariables_DontOverrideExistingIntegration(t *testing.T) {
	...
	input := []string{"something=1", "in=2", "here=3", "W3SECURITY_INTEGRATION_NAME=exists", "W3SECURITY_INTEGRATION_VERSION=already"}
	expected := []string{
		"something=1",
		"in=2",
		"here=3",
		"W3SECURITY_INTEGRATION_NAME=exists",
		"W3SECURITY_INTEGRATION_VERSION=already",
		"HTTP_PROXY=proxy",
		"HTTPS_PROXY=proxy",
		"NODE_EXTRA_CA_CERTS=cacertlocation",
		"W3SECURITY_SYSTEM_NO_PROXY=",
		"W3SECURITY_SYSTEM_HTTP_PROXY=",
		"W3SECURITY_SYSTEM_HTTPS_PROXY=",
		"NO_PROXY=" + constants.W3SECURITY_INTERNAL_NO_PROXY,
	}
...
func Test_PrepareV1EnvironmentVariables_OverrideProxyAndCerts(t *testing.T) {
	...
	expected := []string{
		...
		"HTTPS_PROXY=proxy",
		"NODE_EXTRA_CA_CERTS=cacertlocation",
		"W3SECURITY_SYSTEM_NO_PROXY=312123",
		"W3SECURITY_SYSTEM_HTTP_PROXY=exists",
		"W3SECURITY_SYSTEM_HTTPS_PROXY=already",
		"NO_PROXY=" + constants.W3SECURITY_INTERNAL_NO_PROXY + ",312123",
	}
...
func Test_PrepareV1EnvironmentVariables_Fail_DontOverrideExisting(t *testing.T) {
	...
	input := []string{"something=1", "in=2", "here=3", "W3SECURITY_INTEGRATION_NAME=exists"}
	expected := input
...
func Test_prepareV1Command(t *testing.T) {
	...
	w3securityCmd, err := cliv2.PrepareV1Command(
		"someExecutable",
		expectedArgs,
		getProxyInfoForTest(),
		"name",
	...
	assert.Contains(t, w3securityCmd.Env, "W3SECURITY_INTEGRATION_NAME=name")
	assert.Contains(t, w3securityCmd.Env, "W3SECURITY_INTEGRATION_VERSION=version")
	assert.Contains(t, w3securityCmd.Env, "HTTPS_PROXY=http://w3securitycli:foo@127.0.0.1:1000")
	assert.Contains(t, w3securityCmd.Env, "NODE_EXTRA_CA_CERTS=certLocation")
	assert.Equal(t, expectedArgs, w3securityCmd.Args[1:])
	assert.Nil(t, err)
}
...
func Test_executeEnvironmentError(t *testing.T) {
	...
	os.Setenv(constants.W3SECURITY_INTEGRATION_NAME_ENV, "someName")
...
func Test_executeUnknownCommand(t *testing.T) {
	expectedReturnCode := constants.W3SECURITY_EXIT_CODE_ERROR
