...
import (
	...
	"github.com/w3security/cli/cliv2/internal/constants"
	"github.com/w3security/cli/cliv2/internal/embedded"
	"github.com/w3security/cli/cliv2/internal/embedded/cliv1"
	"github.com/w3security/cli/cliv2/internal/proxy"
	local_utils "github.com/w3security/cli/cliv2/internal/utils"
	"github.com/w3security/go-application-framework/pkg/utils"
)
...
func (c *CLI) GetIntegrationName() string {
	return constants.W3SECURITY_INTEGRATION_NAME
}
...
func PrepareV1EnvironmentVariables(input []string, integrationName string, integrationVersion string, proxyAddress string, caCertificateLocation string) (result []string, err error) {
	...
	_, integrationNameExists := inputAsMap[constants.W3SECURITY_INTEGRATION_NAME_ENV]
	_, integrationVersionExists := inputAsMap[constants.W3SECURITY_INTEGRATION_VERSION_ENV]

	if !integrationNameExists && !integrationVersionExists {
		inputAsMap[constants.W3SECURITY_INTEGRATION_NAME_ENV] = integrationName
		inputAsMap[constants.W3SECURITY_INTEGRATION_VERSION_ENV] = integrationVersion
	} else if !(integrationNameExists && integrationVersionExists) {
		err = EnvironmentWarning{message: fmt.Sprintf("Partially defined environment, please ensure to provide both %s and %s together!", constants.W3SECURITY_INTEGRATION_NAME_ENV, constants.W3SECURITY_INTEGRATION_VERSION_ENV)}
	}

	// preserve original proxy settings
	inputAsMap[constants.W3SECURITY_HTTPS_PROXY_ENV_SYSTEM], _ = utils.FindValueCaseInsensitive(inputAsMap, constants.W3SECURITY_HTTPS_PROXY_ENV)
	inputAsMap[constants.W3SECURITY_HTTP_PROXY_ENV_SYSTEM], _ = utils.FindValueCaseInsensitive(inputAsMap, constants.W3SECURITY_HTTP_PROXY_ENV)
	inputAsMap[constants.W3SECURITY_HTTP_NO_PROXY_ENV_SYSTEM], _ = utils.FindValueCaseInsensitive(inputAsMap, constants.W3SECURITY_HTTP_NO_PROXY_ENV)
	...
	if err == nil {
		...
		blackList := []string{
			constants.W3SECURITY_HTTPS_PROXY_ENV,
			constants.W3SECURITY_HTTP_PROXY_ENV,
			constants.W3SECURITY_CA_CERTIFICATE_LOCATION_ENV,
			constants.W3SECURITY_HTTP_NO_PROXY_ENV,
			constants.W3SECURITY_NPM_NO_PROXY_ENV,
			constants.W3SECURITY_NPM_HTTPS_PROXY_ENV,
			constants.W3SECURITY_NPM_HTTP_PROXY_ENV,
			constants.W3SECURITY_NPM_PROXY_ENV,
			constants.W3SECURITY_NPM_ALL_PROXY,
		}
		...
		inputAsMap[constants.W3SECURITY_HTTPS_PROXY_ENV] = proxyAddress
		inputAsMap[constants.W3SECURITY_HTTP_PROXY_ENV] = proxyAddress
		inputAsMap[constants.W3SECURITY_CA_CERTIFICATE_LOCATION_ENV] = caCertificateLocation

		// merge user defined (external) and internal no_proxy configuration
		if len(inputAsMap[constants.W3SECURITY_HTTP_NO_PROXY_ENV_SYSTEM]) > 0 {
			internalNoProxy := strings.Split(constants.W3SECURITY_INTERNAL_NO_PROXY, ",")
			externalNoProxy := regexp.MustCompile("[,;]").Split(inputAsMap[constants.W3SECURITY_HTTP_NO_PROXY_ENV_SYSTEM], -1)
			mergedNoProxy := utils.Merge(internalNoProxy, externalNoProxy)
			inputAsMap[constants.W3SECURITY_HTTP_NO_PROXY_ENV] = strings.Join(mergedNoProxy, ",")
		} else {
			inputAsMap[constants.W3SECURITY_HTTP_NO_PROXY_ENV] = constants.W3SECURITY_INTERNAL_NO_PROXY
		}
...
func PrepareV1Command(cmd string, args []string, proxyInfo *proxy.ProxyInfo, integrationName string, integrationVersion string) (w3securityCmd *exec.Cmd, err error) {
	proxyAddress := fmt.Sprintf("http://%s:%s@127.0.0.1:%d", proxy.PROXY_USERNAME, proxyInfo.Password, proxyInfo.Port)

	w3securityCmd = exec.Command(cmd, args...)
	w3securityCmd.Env, err = PrepareV1EnvironmentVariables(os.Environ(), integrationName, integrationVersion, proxyAddress, proxyInfo.CertificateLocation)

	return w3securityCmd, err
}

func (c *CLI) executeV1Default(proxyInfo *proxy.ProxyInfo, passthroughArgs []string) error {

	w3securityCmd, err := PrepareV1Command(
		c.v1BinaryLocation,
		passthroughArgs,
		proxyInfo,
	...
	if c.DebugLogger.Writer() != io.Discard {
		...
		variablesMap := utils.ToKeyValueMap(w3securityCmd.Env, "=")
		listedEnvironmentVariables := []string{
			constants.W3SECURITY_CA_CERTIFICATE_LOCATION_ENV,
			constants.W3SECURITY_HTTPS_PROXY_ENV,
			constants.W3SECURITY_HTTP_PROXY_ENV,
			constants.W3SECURITY_HTTP_NO_PROXY_ENV,
			constants.W3SECURITY_HTTPS_PROXY_ENV_SYSTEM,
			constants.W3SECURITY_HTTP_PROXY_ENV_SYSTEM,
			constants.W3SECURITY_HTTP_NO_PROXY_ENV_SYSTEM,
		}
	...
	w3securityCmd.Stdin = c.stdin
	w3securityCmd.Stdout = c.stdout
	w3securityCmd.Stderr = c.stderr
...
func DeriveExitCode(err error) int {
	returnCode := constants.W3SECURITY_EXIT_CODE_OK
	...
	if err != nil {
		if exitError, ok := err.(*exec.ExitError); ok {
			...
			returnCode = constants.W3SECURITY_EXIT_CODE_ERROR
		}
	}
