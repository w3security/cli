...
import (
	...
	"github.com/w3security/cli-extension-sbom/pkg/sbom"
	"github.com/w3security/cli/cliv2/internal/cliv2"
	"github.com/w3security/cli/cliv2/internal/constants"
	"github.com/w3security/cli/cliv2/pkg/basic_workflows"
	"github.com/w3security/go-application-framework/pkg/analytics"
	"github.com/w3security/go-application-framework/pkg/app"
	"github.com/w3security/go-application-framework/pkg/configuration"
	localworkflows "github.com/w3security/go-application-framework/pkg/local_workflows"
	"github.com/w3security/go-application-framework/pkg/workflow"
	"github.com/w3security/go-httpauth/pkg/httpauth"
...
func createCommandsForWorkflows(rootCommand *cobra.Command, engine workflow.Engine) {
	...
	for i := range workflowIdList {
		...
		// last parentCommand is the last level of the command, e.g. in the w3security iac capture workflow, it would be 'capture'
...
func MainWithErrorCode() int {
	...
	if err != nil {
		...
		return constants.W3SECURITY_EXIT_CODE_ERROR
	}
	...
	networkAccess.AddHeaderField("x-w3security-cli-version", cliv2.GetFullVersion())

	extraCaCertFile := config.GetString(constants.W3SECURITY_CA_CERTIFICATE_LOCATION_ENV)
