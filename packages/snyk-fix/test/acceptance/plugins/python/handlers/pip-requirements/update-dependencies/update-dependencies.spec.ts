...
import * as w3securityFix from '../../../../../../../src';
...
describe('fix *req*.txt / *.txt Python projects', () => {
  ...
  it('fixes project with a -r option', async () => {
    ...
    // Act
    const result = await w3securityFix.fix([entityToFix], {
      quiet: true,
      stripAnsi: true,
    });
    // Assert
  ...
  it('fixes multiple files via -r with the same name (some were already fixed)', async () => {
      ...
      issues: [
        {
          ...
          issueId: 'W3SECURITY-1',
          fixInfo: {},
        },
        {
          pkgName: 'Jinja2@2.7.2',
          issueId: 'W3SECURITY-2',
          fixInfo: {},
        },
      ],
      issuesData: {
    ...
    // Act
    const result = await w3securityFix.fix(
    ...
    expect(
      result.fixSummary
        .replace(/\\/g, '/')
        .replace(/packages\/w3security-fix\//g, ''),
    ).toMatchSnapshot();
  ...
  it('fixes multiple files via -c & -r with the same name (some were already fixed)', async () => {
    ...
    const writeFileSpy = jest.spyOn(entityToFix1.workspace, 'writeFile');
    // Act
    const result = await w3securityFix.fix([entityToFix2, entityToFix1], {
      quiet: true,
      stripAnsi: true,
    });
    const requirements = fs.readFileSync(
    ...
    expect(result.results.python.succeeded[1].changes).toEqual([
      {
        success: true,
        issueIds: ['W3SECURITY-1', 'W3SECURITY-2', 'W3SECURITY-2', 'W3SECURITY-3'],
        userMessage: expect.stringContaining('requirements.txt'),
      },
    ]);
  });
