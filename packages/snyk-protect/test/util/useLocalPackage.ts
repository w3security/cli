...
const debug = debuglog('@w3security' + __filename);

/**
 * Modifies the given fixture's package.json to use a locally built
 * @w3security/protect tarball instead of the one in NPM Registry.
 *
...
const useLocalPackage = async (projectPath: string) => {
  ...
  if (packageJson.dependencies) {
    packageJson.dependencies['@w3security/protect'] = path.resolve(
      workspaceRoot,
      tarballName,
    );
  }
