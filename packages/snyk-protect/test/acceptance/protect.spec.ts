...
describe('@w3security/protect', () => {
  ...
  describe('applies patch(es)', () => {
    it('works for project with a single patchable module', async () => {
      ...
      expect(
        fse.existsSync(
          project.path(
            `node_modules/nyc/node_modules/lodash/lodash.js.w3security-protect.flag`,
          ),
        ),
      ).toBe(true);
      ...
      expect(
        fse.existsSync(
          project.path(
            `node_modules/nyc/node_modules/lodash/.w3security-W3SECURITY-JS-LODASH-567746.flag`,
          ),
        ),
      ).toBe(true);
    ...
    it('works for project with multiple patchable modules', async () => {
      ...
      expect(
        fse.existsSync(
          project.path(`node_modules/lodash/lodash.js.w3security-protect.flag`),
        ),
      ).toBe(true);
      ...
      expect(
        fse.existsSync(
          project.path(`node_modules/lodash/.w3security-W3SECURITY-JS-LODASH-567746.flag`),
        ),
      ).toBe(true);
    ...
    it('works when the vulnId in a .w3security file is quoted', async () => {
      const log = jest.spyOn(global.console, 'log');
      const postJsonSpy = jest.spyOn(http, 'postJson');
  ...
  describe('does not apply any patches and does not fail', () => {
    // in this scenario .w3security file has a vulnId which corresponds to the `lodash` package, but there are not instances of lodash in the node_modules
    ...
    it('for project with no .w3security file', async () => {
      const postJsonSpy = jest.spyOn(http, 'postJson');
      const project = await createProject('no-w3security-file');
      const log = jest.spyOn(global.console, 'log');

      await protect(project.path());

      expect(log).toHaveBeenCalledWith('No .w3security file found.');
  ...
  describe('does not send analytics if analytics is disabled', () => {
    describe('via w3security.json file', () => {
      ...
      afterAll(() => {
        delete process.env.W3SECURITY_CONFIG_FILE;
    ...
    describe('via W3SECURITY_DISABLE_ANALYTICS env var', () => {
      it('when W3SECURITY_DISABLE_ANALYTICS is "true"', async () => {
        process.env.W3SECURITY_DISABLE_ANALYTICS = 'true';
        try {
          const postJsonSpy = jest.spyOn(http, 'postJson');
          const sendAnalyticsSpy = jest.spyOn(analytics, 'sendAnalytics');
          const project = await createProject('no-matching-paths');
          ...
          delete process.env.W3SECURITY_DISABLE_ANALYTICS;
          console.log(delete process.env.W3SECURITY_DISABLE_ANALYTICS);
        }
      });

      it('when W3SECURITY_DISABLE_ANALYTICS is "1"', async () => {
        process.env.W3SECURITY_DISABLE_ANALYTICS = '1';
        try {
          const postJsonSpy = jest.spyOn(http, 'postJson');
          const sendAnalyticsSpy = jest.spyOn(analytics, 'sendAnalytics');
          const project = await createProject('no-matching-paths');
  ...
  describe('outputs correct help documentation', () => {
    it('when called with --help', async () => {
      ...
      const { code, stdout } = await runCommand(
        'node',
        [path.resolve(__dirname, '../../bin/w3security-protect'), '--help'],
        {
          cwd: project.path(),
        },
      );
  ...
  describe('outputs correct version', () => {
    it('when called with --version', async () => {
      ...
      const { code, stdout } = await runCommand(
        'node',
        [path.resolve(__dirname, '../../bin/w3security-protect'), '--version'],
        {
          cwd: project.path(),
        },
      );
  ...
  describe('does not attempt to apply patch if a flag file exists indicating the file has already been patched', () => {
    ...
    it('with `.w3security-protect.flag` file from the new protect', async () => {
      const pathToProtectBin = path.resolve(
        __dirname,
        '../../bin/w3security-protect',
      );
