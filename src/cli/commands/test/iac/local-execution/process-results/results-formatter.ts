...
import { SEVERITY, SEVERITIES } from '../../../../../../lib/w3security-test/common';
  ...
  const formattedIssues = scanResult.violatedPolicies.map((policy) => {
    ...
    return {
      ...
      documentation: !isGeneratedByCustomRule
        ? `https://w3security.io/security-rules/${policy.publicId}`
        : undefined,
...
export class FailedToFormatResults extends CustomError {
  constructor(message?: string) {
    ...
    this.userMessage =
      'We failed printing the results, please contact support@w3security.io';
  }
}
