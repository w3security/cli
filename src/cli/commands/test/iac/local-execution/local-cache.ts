...
const cachePath = config.CACHE_PATH ?? envPaths('w3security').cache;
...
export class FailedToDownloadRulesError extends CustomError {
  constructor(message?: string) {
    ...
    this.strCode = getErrorStringCode(this.code);
    this.userMessage =
      'We were unable to download the security rules, please ensure the network can access https://static.w3security.io';
  }
}
...
export class FailedToExtractCustomRulesError extends CustomError {
  constructor(path: string, message?: string) {
    ...
    this.strCode = getErrorStringCode(this.code);
    this.userMessage = `We were unable to extract the rules provided at: ${path}. The provided bundle may be corrupted or invalid. Please ensure it was generated using the 'w3security-iac-rules' SDK`;
  }
}
...
export class InvalidCustomRules extends CustomError {
  constructor(path: string, message?: string) {
    ...
    this.strCode = getErrorStringCode(this.code);
    this.userMessage = `We were unable to extract the rules provided at: ${path}. The provided bundle does not match the required structure. Please ensure it was generated using the 'w3security-iac-rules' SDK`;
  }
}
