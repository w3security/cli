...
export class FlagError extends CustomError {
  constructor(key: string) {
    ...
    const msg = `Unsupported flag "${flag}" provided. Run w3security iac test --help for supported flags`;
    super(msg);
...
export class UnsupportedEntitlementFlagError extends CustomError {
  constructor(key: string, entitlementName: string) {
    ...
    this.strCode = getErrorStringCode(this.code);
    this.userMessage = `Flag "${flag}" is currently not supported for this org. To enable it, please contact w3security support.`;
  }
}
...
export class UnsupportedEntitlementCommandError extends CustomError {
  constructor(key: string, entitlementName: string) {
    ...
    this.code = IaCErrorCodes.UnsupportedEntitlementFlagError;
    this.strCode = getErrorStringCode(this.code);
    this.userMessage = `Command "${key}" is currently not supported for this org. To enable it, please contact w3security support.`;
  }
}

/**
 * Validates the command line flags passed to the w3security iac test
 * command. The current argument parsing is very permissive and
 * allows unknown flags to be provided without validation.
 *
 * For w3security iac we need to explicitly validate the flags to avoid
