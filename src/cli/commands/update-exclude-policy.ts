...
export default async (...args: MethodArgs): Promise<any> => {
  ...
  // Ensure that this update-exclude-policy command can only be runned when using `w3security iac update-exclude-policy`
  // Avoid `w3security update-exclude-policy` direct usage
  if (options.iac != true) {
  ...
  try {
    ...
    let policy: Policy;
    try {
      policy = await w3securityPolicyLib.load();
    } catch (error) {
      if (error.code === 'ENOENT') {
        // policy file does not exist - create it
        policy = await w3securityPolicyLib.create();
      } else {
        throw error;
      }
    }
    await updateExcludeInPolicy(policy, analysis, options);
    await w3securityPolicyLib.save(policy);
  } catch (e) {
