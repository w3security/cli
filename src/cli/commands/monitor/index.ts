...
import { monitor as w3securityMonitor } from '../../../lib/monitor';
import { processJsonMonitorResponse } from './process-json-monitor';
import w3security = require('../../../lib'); // TODO(kyegupov): fix import
  ...
  for (const path of paths) {
    ...
    try {
      ...
      if (unsupportedPackageManager) {
        return `${unsupportedPackageManager.label} projects do not currently support "w3security monitor"`;
      }
      const targetFile =
        !options.scanAllUnmanaged && options.docker && !options.file // w3security monitor --docker (without --file)
          ? undefined
        ...
        try {
          if (!projectDeps.depGraph && !projectDeps.depTree) {
            ...
            throw new FailedToRunTestError(
              'Your monitor request could not be completed. Please email support@w3security.io',
            );
          }
...
function validateMonitorPath(path: string, isDocker?: boolean): void {
  ...
  if (!exists && !isDocker) {
    throw new Error('"' + path + '" is not a valid path for "w3security monitor"');
  }
}
