...
const debug = debugModule('w3security:run-test');
...
async function parseRes(
  ...
  if (!options['ignore-policy']) {
    ...
    const policy = await w3security.policy.loadFromText(res.policy);
...
async function assembleRemotePayloads(root, options): Promise<Payload[]> {
  ...
  // options.vulnEndpoint is only used by `w3security protect` (i.e. local filesystem tests)
