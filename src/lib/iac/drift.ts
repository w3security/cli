...
import w3securityLogoSVG from './assets/w3security-logo';
import w3securityFaviconBase64 from './assets/w3security-favicon';
...
function rebrandHTMLOutput(data: string): string {
  ...
  data = data.replace(
    faviconReplaceRegex,
    `<link rel="shortcut icon" type="image/x-icon" href="${w3securityFaviconBase64}" />`,
  );
  ...
  data = data.replace(
    logoReplaceRegex,
    `<div id="brand_logo">${w3securityLogoSVG}</div>`,
  );
