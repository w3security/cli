...
const defaultUserMessage =
  'Your test request could not be completed. Please run the command again with the `-d` flag and contact support@w3security.io with the contents of the output';

const w3securityIacTestErrorsUserMessages = {
...
export function getErrorUserMessage(code: number, error: string): string {
  if (code < 2000 || code >= 3000) {
    return 'INVALID_W3SECURITY_IAC_TEST_ERROR';
  }
