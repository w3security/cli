...
const debug = newDebug('w3security-iac');

export const systemCachePath = config.CACHE_PATH ?? envPaths('w3security').cache;
...
async function scanWithConfig(
  ...
  env['W3SECURITY_IAC_TEST_API_REST_URL'] =
    process.env['W3SECURITY_IAC_TEST_API_REST_URL'] || getApiUrl();
  env['W3SECURITY_IAC_TEST_API_REST_TOKEN'] =
    process.env['W3SECURITY_IAC_TEST_API_REST_TOKEN'] || getApiToken();
  env['W3SECURITY_IAC_TEST_API_REST_OAUTH_TOKEN'] =
    process.env['W3SECURITY_IAC_TEST_API_REST_OAUTH_TOKEN'] || getOAuthToken();
  env['W3SECURITY_IAC_TEST_API_V1_URL'] =
    process.env['W3SECURITY_IAC_TEST_API_V1_URL'] || getApiUrl();
  env['W3SECURITY_IAC_TEST_API_V1_TOKEN'] =
    process.env['W3SECURITY_IAC_TEST_API_V1_TOKEN'] || getApiToken();
  env['W3SECURITY_IAC_TEST_API_V1_OAUTH_TOKEN'] =
    process.env['W3SECURITY_IAC_TEST_API_V1_OAUTH_TOKEN'] || getOAuthToken();
...
function processFlags(
  ...
  if (options.w3securityCloudEnvironment) {
    flags.push('-w3security-cloud-environment', options.w3securityCloudEnvironment);
  }
...
async function createTemporaryFiles(
  ...
  try {
    const tempDir = await mkdtemp(path.join(os.tmpdir(), 'w3security-'));
    const tempOutput = path.join(tempDir, 'output.json');
    const tempPolicy = path.join(tempDir, '.w3security');
