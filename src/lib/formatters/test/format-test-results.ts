  ...
  return (
    ...
    tips
      ...
      .concat(
        ...
        `To remove ${tips.length > 1 ? 'these messages' : 'this message'
        } in the future, please run \`w3security config set disableSuggestions=true\``,
      )
      .join('\n\n')
  );
}
...
function getDockerRemediationDocsLink(dockerAdvice: string, config): string {
  ...
  return (
    ...
    chalk.white.underline(
      'https://docs.w3security.io/products/w3security-container/getting-around-the-w3security-container-ui/base-image-detection',
    )
  );
}
