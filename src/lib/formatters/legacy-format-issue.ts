...
import { parsePackageString as w3securityModule } from 'snyk-module';
import {
...
import {
  ...
  DockerIssue,
  SEVERITY,
} from '../../lib/w3security-test/legacy';
import { formatLegalInstructions } from './legal-license-instructions';
import { colorTextBySeverity } from '../../lib/w3security-test/common';
    ...
    const upgradePathsArray = uniq(
      vuln.list.map((v) => {
        ...
        if (shouldUpgradeItself) {
          ...
          // Example command: w3security test express@3
          const selfUpgradeInfo =
            v.upgradePath.length > 0
              ? ` (triggers upgrades to ${v.upgradePath.join(PATH_SEPARATOR)})`
              : '';
          const testedPackageName = w3securityModule(v.upgradePath[0] as string);
          return (
