...
export function formatSnykIacTestTestData(
  ...
  const allFilesCount = countFiles(w3securityIacTestScanResult);
  const filesWithIssuesCount = countFilesWithIssues(w3securityIacTestScanResult);
  const filesWithoutIssuesCount = allFilesCount - filesWithIssuesCount;
  const ignores = w3securityIacTestScanResult
    ? w3securityIacTestScanResult.metadata.ignoredCount
    : 0;

  let contextSuppressedIssueCount: number | undefined;
  const suppressedResults =
    w3securityIacTestScanResult?.scanAnalytics?.suppressedResults;
...
function formatSnykIacTestScanResultNewOutput(
  ...
  if (w3securityIacTestScanResult?.vulnerabilities) {
    w3securityIacTestScanResult.vulnerabilities.forEach((vulnerability) => {
