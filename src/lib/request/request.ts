...
const debug = debugModule('w3security:req');
const w3securityDebug = debugModule('w3security');
...
function setupRequest(payload: Payload) {
  ...
  payload.headers['x-w3security-cli-version'] = versionNumber;
  ...
  if (body && !noCompression) {
    ...
    w3securityDebug('sending request to:', payload.url);
    w3securityDebug('request body size:', json.length);
    w3securityDebug('gzipped request body size:', data.length);

    let callGraphLength: number | null = null;
    if (body.callGraph) {
      callGraphLength = JSON.stringify(body.callGraph).length;
      w3securityDebug('call graph size:', callGraphLength);
    }
  ...
  if (
    ...
    process.env.W3SECURITY_HTTP_PROTOCOL_UPGRADE !== '0'
  ) {
  ...
  if (proxyUri) {
    w3securityDebug('using proxy:', proxyUri);
    bootstrap({
      environmentVariableNamespace: '',
    });
  } else {
    w3securityDebug('not using proxy');
  }
