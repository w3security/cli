...
const debug = createDebug('w3security-spotlight-vuln-notification');

const spotlightVulnIds = ['W3SECURITY-JAVA-ORGAPACHELOGGINGLOG4J-2314720'];
  ...
  try {
    if (foundSpotlightVulnsIds.length > 0) {
      ...
      for (const vulnId of spotlightVulnIds) {
        if (vulnId === 'W3SECURITY-JAVA-ORGAPACHELOGGINGLOG4J-2314720') {
          ...
          message +=
            ...
            EOL +
            `  - ${config.PUBLIC_VULN_DB_URL}/vuln/W3SECURITY-JAVA-ORGAPACHELOGGINGLOG4J-2314720` +
            EOL +
