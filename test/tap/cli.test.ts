...
test('test without authentication', async (t) => {
  ...
  try {
    ...
    t.match(
      error.message,
      '`w3security` requires an authenticated account. Please run `w3security auth` and try again.',
...
test('auth via key', async (t) => {
  try {
    ...
    t.notEqual(res.toLowerCase().indexOf('ready'), -1, 'w3security auth worked');
  } catch (e) {
    t.threw(e);
  }
});
...
test('auth with UTMs in environment variables', async (t) => {
  ...
  process.env.W3SECURITY_UTM_MEDIUM = 'ide';
  process.env.W3SECURITY_UTM_SOURCE = 'eclipse';
  process.env.W3SECURITY_UTM_CAMPAIGN = 'plugin';
  ...
  try {
    ...
    delete process.env.W3SECURITY_UTM_MEDIUM;
    delete process.env.W3SECURITY_UTM_SOURCE;
    delete process.env.W3SECURITY_UTM_CAMPAIGN;
    // clean up stubs
    ciStub.restore();
    dockerStub.restore();
...
test('w3security ignore - all options', async (t) => {
...
test('w3security ignore - no ID', async (t) => {
  try {
    const dir = await makeTmpDirectory();
    await cli.ignore({
      reason: 'REASON',
...
test('w3security ignore - default options', async (t) => {
  const clock = sinon.useFakeTimers(new Date(2016, 11, 1).getTime());
  try {
    const dir = await makeTmpDirectory();
    await cli.ignore({
...
test('w3security ignore - not authorized', async (t) => {
  const dir = await makeTmpDirectory();
...
test('w3security policy', async (t) => {
  await cli.policy();
