...
test('prime config', async (t) => {
  ...
  process.env.W3SECURITY_DOCKER_TOKEN = 'docker-jwt-token';
  t.pass('docker token set');

  t.end();
});

test('`w3security test` with docker flag - docker token and no api key', async (t) => {
...
test('`w3security test` with docker flag - docker token and api key', async (t) => {
  stubDockerPluginResponse(
    plugins,
    {
      scanResults: [
...
test('`w3security test` without docker flag - docker token and no api key', async (t) => {
  stubDockerPluginResponse(
    plugins,
    {
      scanResults: [
...
test('`w3security test` with docker flag - displays CTA', async (t) => {
  stubDockerPluginResponse(
    plugins,
    {
      scanResults: [
...
test('`w3security test` with docker flag - does not display CTA', async (t) => {
  stubDockerPluginResponse(
    plugins,
    {
      scanResults: [
...
test('teardown', async (t) => {
  ...
  delete process.env.W3SECURITY_DOCKER_TOKEN;
