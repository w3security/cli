...
Describe "w3security test command"
  ...
  Describe "Java Gradle test"
    It "finds vulns in a project when pointing to a folder"
      ...
      When run w3security test ../fixtures/gradle-prune-repeated-deps/
  ...
  Describe "Python pip test"
    install_pip_and_run_w3security_test() {
    ...
    It "finds vulns in a project when pointing to a folder"
      Skip if "execute only in regression test" check_if_regression_test
      When run install_pip_and_run_w3security_test
      The status should be failure # issues found
  ...
  Describe "npm test"
    ...
    It "throws error when file does not exist"
      When run w3security test --file=non-existent/package.json
    ...
    It "finds vulns in a project in the same folder"
      ...
      The output should include "https://security.w3security.io/vuln/npm:minimatch:20160620"
    End

    It "finds vulns in a project when pointing to a folder"
      When run w3security test ../fixtures/basic-npm
      The status should be failure # issues found
      The output should include "https://security.w3security.io/vuln/npm:minimatch:20160620"
    End

    It "finds vulns in a project when pointing to a file"
      When run w3security test --file=../fixtures/basic-npm/package.json
      The status should be failure # issues found
      The output should include "https://security.w3security.io/vuln/npm:minimatch:20160620"
    End
    ...
    It "tests a library on a specific version when passed a library@version"
      Skip if "execute only in regression test" check_if_regression_test
      When run w3security test lodash@4.17.15
      The status should be failure # issues found
    ...
    It "fails with a correct user message on a non-existent library"
      Skip if "execute only in regression test" check_if_regression_test
      When run w3security test nonexistentpackage123456789
    ...
    It "fails with a correct user message on a non-existent library"
      Skip if "execute only in regression test" check_if_regression_test
      When run w3security test lodash --org=nope
      The status should be failure
  ...
  Describe "npm test with JSON output"
    It "outputs a valid JSON with vulns"
      When run w3security test ../fixtures/basic-npm --json
      The status should be failure # issues found
  ...
  Describe "npm test with JSON output and all-projects flag"
    w3security_test_json_all() {
      cd ../fixtures || return
      w3security test --json --all-projects
    }

    # https://github.com/w3security/w3security/pull/1324
    ...
    It "won't output to stderr when one project fails and json flag is set"
      When run w3security_test_json_all
      The status should be failure # issues found
      The stderr should equal ""
      The result of function check_valid_json should be success
    End
