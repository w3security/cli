...
describe('Sanitize args', () => {
  it('should obfuscate username and password when both are provided', () => {
    const argsWithUsernameAndPassword: ArgsOptions = {
      _doubleDashArgs: [],
      _: ['w3security/goof-image:latest'],
      org: 'demo-org',
      username: 'fakeuser',
      password: 'fakepass',
      file: 'Dockerfile',
    ...
    expect(resultWithFlag._[0]).toEqual('w3security/goof-image:latest');
  ...
  it('should obfuscate nested PII', () => {
    ...
    expect(resultWithFlag[0]).toEqual('w3security/goof-image:latest');
