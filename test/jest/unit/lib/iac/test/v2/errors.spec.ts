...
describe('getErrorUserMessage', () => {
  it('returns INVALID_W3SECURITY_IAC_TEST_ERROR for an invalid w3security-iac-test error code', () => {
    expect(getErrorUserMessage(0, '')).toEqual('INVALID_W3SECURITY_IAC_TEST_ERROR');
    expect(getErrorUserMessage(3000, '')).toEqual(
      'INVALID_W3SECURITY_IAC_TEST_ERROR',
    );
  });
  ...
  it.each`
    ...
    ${2114}
  `(
    'returns a user message for a valid w3security-iac-test error code - $expectedErrorCode',
    ({ expectedErrorCode }) => {
