...
describe('validateResultFromCustomRules', () => {
  const result: IacFileScanResult = {
    ...
    violatedPolicies: [
      ...
      {
        publicId: 'W3SECURITY-CC-CUSTOM-RULE-INVALID',
        subType: '',
        title: '',
        severity: SEVERITY.LOW,
        msg: 'input.resource',
  ...
  it('filters out policies with conflicting publicId', () => {
    ...
    expect(validatedResult.violatedPolicies).not.toEqual(
      expect.arrayContaining([
        expect.objectContaining({ publicId: 'W3SECURITY-CC-CUSTOM-RULE-INVALID' }),
      ]),
    );
    expect(invalidIssues.length).toEqual(3);
    expect(invalidIssues).toEqual(
      expect.arrayContaining([
        expect.objectContaining({
          failureReason:
            'Invalid publicId for custom rule W3SECURITY-CC-CUSTOM-RULE-INVALID. Change to a publicId that does not start with W3SECURITY-CC-',
        }),
      ]),
    );
  });
