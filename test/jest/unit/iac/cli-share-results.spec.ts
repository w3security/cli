...
import * as w3securityPolicyLib from 'snyk-policy';

describe('CLI Share Results', () => {
  let w3securityPolicy: Policy;
  ...
  beforeAll(async () => {
    w3securityPolicy = await w3securityPolicyLib.load('test/jest/unit/iac/fixtures');
  });
  ...
  it("converts the results to Envelope's ScanResult interface - without .w3security policies", async () => {
    await shareResults({
      results: scanResults,
      policy: undefined,
      meta: {
        ...
        orgName: 'org-name',
        gitRemoteUrl: 'http://github.com/w3security/cli.git',
      },
      options: {},
    });
  ...
  it("converts the results to Envelope's ScanResult interface - with .w3security policies", async () => {
    await shareResults({
      results: scanResults,
      policy: w3securityPolicy,
      meta: {
        projectName: 'project-name',
        orgName: 'org-name',
        gitRemoteUrl: 'http://github.com/w3security/cli.git',
