...
describe('assertIntegratedIaCOnlyOptions()', () => {
  ...
  it('Refuses w3security-cloud-environment flag', () => {
    const options = ['--w3security-cloud-environment', 'envid'];
    expect(() =>
      assertIntegratedIaCOnlyOptions(org, [...command, ...options, ...files]),
    ).toThrow(new IntegratedFlagError('w3security-cloud-environment', org.meta.org));
  });
});
