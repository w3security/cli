...
describe('w3security test --all-projects with one project that has errors', () => {
  let server;
  ...
  describe('and another that has issues (vulnerabilities)', () => {
    it('should exit with exit code 1 when the `--fail-fast` option is not set', async () => {
      const project = await createProject(
        'w3security-test-all-projects-exit-codes/project-with-issues-and-project-with-error',
      );
