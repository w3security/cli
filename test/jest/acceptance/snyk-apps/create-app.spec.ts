...
describe('w3security-apps: create app', () => {
  ...
  beforeAll((done) => {
    ...
    env = {
      ...
      W3SECURITY_API_REST_URL: 'http://localhost:' + port + baseApi,
  ...
  describe('scriptable mode', () => {
    ...
    it('throws an error when an invalid context is provided', async () => {
      ...
      await expect(cli).toDisplay(
        "Option '--context' must be either 'tenant' or 'user'! For interactive mode, please use '--interactive' or '-i' flag. For more information please run the help command 'w3security apps --help' or 'w3security apps -h'.",
      );
      await expect(cli).toExitWith(2);
    });
