...
describe('iac test --rules', () => {
  ...
  it('scans local custom rules provided via the --rules flag', async () => {
    const { stdout, exitCode } = await run(
      `w3security iac test --rules=./iac/custom-rules/custom.tar.gz ./iac/terraform/sg_open_ssh.tf`,
    );
  ...
  it('does not show custom rules message if it scans local custom rules and uses --json flag', async () => {
    const { stdout, exitCode } = await run(
      `w3security iac test --rules=./iac/custom-rules/custom.tar.gz ./iac/terraform/sg_open_ssh.tf --json`,
    );
  ...
  it('presents an error message when the local rules cannot be found', async () => {
    const { stdout, exitCode } = await run(
      `w3security iac test --rules=./not/a/real/path.tar.gz ./iac/terraform/sg_open_ssh.tf`,
    );
    expect(stdout).toContain(
  ...
  it('presents an error message when the user is not entitled to custom-rules', async () => {
    const { stdout, exitCode } = await run(
      `w3security iac test --org=no-custom-rules-entitlements --rules=./iac/custom-rules/custom.tar.gz ./iac/terraform/sg_open_ssh.tf`,
    );
    expect(stdout).toContain(
      `Flag "--rules" is currently not supported for this org. To enable it, please contact w3security support.`,
    );
    expect(exitCode).toBe(2);
  });
  ...
  describe('when used with the --report flag', () => {
    it('should resolve successfully', async () => {
      const { stderr, exitCode } = await run(
        `w3security iac test --report --rules=./iac/custom-rules/custom.tar.gz ./iac/terraform/sg_open_ssh.tf`,
      );
    ...
    describe.each(['--json', '--sarif'])(
      ...
      (testedFormatFlag) => {
        it('should not display the warning message for the custom rules not being available on the platform', async () => {
          ...
          const { stdout } = await run(
            `w3security iac test --report --rules=./iac/custom-rules/custom.tar.gz ./iac/terraform/sg_open_ssh.tf ${testedFormatFlag}`,
          );
