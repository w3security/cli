...
describe('CloudFormation single file scan', () => {
  ...
  it('finds issues in CloudFormation YAML file', async () => {
    const { stdout, exitCode } = await run(
      `w3security iac test ./iac/cloudformation/aurora-valid.yml`,
    );
  ...
  it('finds issues in CloudFormation JSON file', async () => {
    const { stdout, exitCode } = await run(
      `w3security iac test ./iac/cloudformation/fargate-valid.json`,
    );
    expect(stdout).toContain(
  ...
  it('filters out issues when using severity threshold', async () => {
    const { stdout, exitCode } = await run(
      `w3security iac test ./iac/cloudformation/aurora-valid.yml --severity-threshold=high`,
    );
  ...
  it('outputs an error for files with no valid YAML', async () => {
    const { stdout, exitCode } = await run(
      `w3security iac test ./iac/cloudformation/invalid-cfn.yml`,
    );
  ...
  it('outputs the expected text when running with --sarif flag', async () => {
    const { stdout, exitCode } = await run(
      `w3security iac test ./iac/cloudformation/aurora-valid.yml --sarif`,
    );

    expect(isValidJSONString(stdout)).toBe(true);
    expect(stdout).toContain('"id": "W3SECURITY-CC-TF-55",');
    expect(stdout).toContain('"ruleId": "W3SECURITY-CC-TF-55",');
    expect(exitCode).toBe(1);
  });

  it('outputs the expected text when running with --json flag', async () => {
    const { stdout, exitCode } = await run(
      `w3security iac test ./iac/cloudformation/aurora-valid.yml --json`,
    );
