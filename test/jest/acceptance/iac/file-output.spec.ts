...
import { MappedIacTestResponse } from '../../../../src/lib/w3security-test/iac-test-result';
...
describe('iac test --json-file-output', () => {
  ...
  it('contains a valid line number', async () => {
    ...
    const { stdout } = await run(
      `w3security iac test ./iac/terraform/sg_open_ssh.tf --json-file-output=${jsonOutputFilename}`,
    );
  ...
  it('returns the correct paths for provided path', async () => {
    const jsonOutputFilename = path.join(__dirname, `${uuidv4()}.json`);
    const { stdout } = await run(
      `w3security iac test ./iac/file-output/sg_open_ssh.tf --json-file-output=${jsonOutputFilename}`,
    );
    expect(stdout).toMatch('Organization:');
...
describe('iac test --sarif-file-output', () => {
  ...
  it('contains a valid line number', async () => {
    ...
    const { stdout } = await run(
      `w3security iac test ./iac/terraform/sg_open_ssh.tf --sarif-file-output=${sarifOutputFilename}`,
    );
    expect(stdout).toMatch('Organization:');
  ...
  it('returns the correct paths for provided path', async () => {
    const sarifOutputFilename = path.join(__dirname, `${uuidv4()}.sarif`);
    const { stdout } = await run(
      `w3security iac test ./iac/file-output/sg_open_ssh.tf --sarif-file-output=${sarifOutputFilename}`,
    );
    expect(stdout).toMatch('Organization:');
  ...
  it('does not include file content in analytics logs', async () => {
    const { stdout, exitCode } = await run(
      `w3security iac test ./iac/file-logging -d`,
    );
