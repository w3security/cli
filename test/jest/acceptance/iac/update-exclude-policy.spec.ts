...
describe('iac update-exclude-policy', () => {
  ...
  it('update-exclude-policy fail without the right entitlement', async () => {
    const { stdout, stderr, exitCode } = await run(
      `w3security iac update-exclude-policy --org=no-iac-drift-entitlements`,
      {},
    );

    expect(stdout).toMatch(
      'Command "update-exclude-policy" is currently not supported for this org. To enable it, please contact w3security support.',
    );
  ...
  it('update-exclude-policy successfully executed when org has the entitlement', async () => {
    ...
    const w3securityBinaryPath = path.join(__dirname, '../../../../bin/w3security');

    const { stderr, stdout, exitCode } = await run(
      `cat ${analysisPath} | ${w3securityBinaryPath} iac update-exclude-policy`,
      {},
      tmpFolderPath,
    );
