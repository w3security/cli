...
describe('iac test with infrastructureAsCode entitlement', () => {
  ...
  it('fails to scan because the user is not entitled to infrastructureAsCode', async () => {
    const { stdout, exitCode } = await run(
      `w3security iac test --org=no-iac-entitlements ./iac/terraform/sg_open_ssh.tf`,
    );
    expect(stdout).toContain(
      'This feature is currently not enabled for your org. To enable it, please contact w3security support.',
    );
