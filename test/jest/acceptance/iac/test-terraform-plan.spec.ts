...
describe('Terraform plan scanning', () => {
  ...
  it('finds issues in a Terraform plan file', async () => {
    const { stdout, exitCode } = await run(
      `w3security iac test ./iac/terraform-plan/tf-plan-create.json`,
    );
  ...
  it('finds issues in a Terraform plan file - explicit delta scan with flag', async () => {
    const { stdout, exitCode } = await run(
      `w3security iac test ./iac/terraform-plan/tf-plan-create.json --scan=resource-changes`,
    );
  ...
  it('errors when a wrong value is passed to the --scan flag', async () => {
    const { stdout, exitCode } = await run(
      `w3security iac test ./iac/terraform-plan/tf-plan-create.json --scan=resrc-changes`,
    );
    expect(stdout).toContain(
  ...
  it('errors when no value is provided to the --scan flag', async () => {
    const { stdout, exitCode } = await run(
      `w3security iac test ./iac/terraform-plan/tf-plan-create.json.json  --scan`,
    );
    expect(stdout).toContain(
  ...
  it('succesfully scans a TF-Plan with the --json output flag', async () => {
    const { stdout, exitCode } = await run(
      `w3security iac test ./iac/terraform-plan/tf-plan-create.json --json`,
    );
