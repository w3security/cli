...
describe('CLI Share Results', () => {
  ...
  describe('feature flag is not enabled', () => {
    ...
    it('the output includes an error', async () => {
      const { stdout, exitCode } = await run(
        `w3security iac test ./iac/arm/rule_test.json --report`,
      );
  ...
  describe('feature flag is enabled', () => {
    ...
    it('forwards project tags', async () => {
      const { exitCode } = await run(
        'w3security iac test ./iac/arm/rule_test.json --report --tags=foo=bar',
      );
    ...
    it('forwards project environment', async () => {
      const { exitCode } = await run(
        'w3security iac test ./iac/arm/rule_test.json --report --project-environment=saas',
      );
    ...
    it('forwards project lifecycle', async () => {
      const { exitCode } = await run(
        'w3security iac test ./iac/arm/rule_test.json --report --project-lifecycle=sandbox',
      );
    ...
    it('forwards project business criticality', async () => {
      const { exitCode } = await run(
        'w3security iac test ./iac/arm/rule_test.json --report --project-business-criticality=high',
      );
    ...
    it('should print an error message if test usage is exceeded', async () => {
      ...
      const { stdout, exitCode } = await run(
        'w3security iac test ./iac/arm/rule_test.json --report --project-business-criticality=high --org=aaaaaaaa-bbbb-cccc-dddd-eeeeeeeeeeee',
      );
    ...
    it('should filter out NONE custom policies severity issues and then forward', async () => {
      const { exitCode } = await run(
        'w3security iac test ./iac/arm/rule_test.json --report --org=custom-policies',
      );
      ...
      // The other W3SECURITY-CC-AZURE-543 issue has been filtered out
    ...
    describe('with target reference', () => {
      it('forwards the target reference to iac-cli-share-results endpoint', async () => {
        ...
        const { exitCode } = await run(
          `w3security iac test ./iac/arm/rule_test.json --report --target-reference=${testTargetRef}`,
        );
