...
const paths = envPaths('w3security');
...
describe('iac describe', () => {
  ...
  it('describe fail when not used as an iac sub-command', async () => {
    const { stdout, stderr, exitCode } = await run(`w3security describe`, {});
  ...
  it('describe fail without the right entitlement', async () => {
    const { stdout, stderr, exitCode } = await run(
      `w3security iac describe --all --org=no-iac-drift-entitlements`,
      {
        W3SECURITY_DRIFTCTL_PATH: './iac/drift/args-echo',
      },
    );

    expect(stdout).toMatch(
      'Command "drift" is currently not supported for this org. To enable it, please contact w3security support.',
    );
    expect(stderr).toMatch('');
    expect(exitCode).toBe(2);
  });
  ...
  it('Test when driftctl scan exit in error', async () => {
    const { stdout, stderr, exitCode } = await run(`w3security iac describe  --all`, {
      W3SECURITY_FIXTURE_OUTPUT_PATH: outputFile,
      DCTL_EXIT_CODE: '2',
      W3SECURITY_DRIFTCTL_PATH: path.join(
        getFixturePath('iac'),
        'drift',
        'args-echo.sh',
      ),
  ...
  it('Launch driftctl from W3SECURITY_DRIFTCTL_PATH env var when org has the entitlement', async () => {
    const { stdout, stderr, exitCode } = await run(`w3security iac describe  --all`, {
      W3SECURITY_FIXTURE_OUTPUT_PATH: outputFile,
      W3SECURITY_DRIFTCTL_PATH: path.join(
        getFixturePath('iac'),
    ...
    expect(output).toContain('DCTL_IS_W3SECURITY=true');
  ...
  it('Launch driftctl from W3SECURITY_DRIFTCTL_PATH with html output', async () => {
    const { stdout, stderr, exitCode } = await run(
      `w3security iac describe  --all --html`,
      {
        W3SECURITY_FIXTURE_OUTPUT_PATH: outputFile,
        W3SECURITY_DRIFTCTL_PATH: path.join(
          getFixturePath('iac'),
  ...
  describe('Test html file output', () => {
    ...
    it('Launch driftctl with html output format', async () => {
      const { stdout, stderr, exitCode } = await run(
        `w3security iac describe --all --html-file-output=${htmlFile}`,
        {
          W3SECURITY_FIXTURE_OUTPUT_PATH: outputFile,
          W3SECURITY_DRIFTCTL_PATH: path.join(
            getFixturePath('iac'),
  ...
  it('Download and launch driftctl when executable is not found and org has the entitlement', async () => {
    ...
    const { stderr, exitCode } = await run(`w3security iac describe --all`, {
      W3SECURITY_DRIFTCTL_URL: apiUrl + '/download/driftctl',
      W3SECURITY_CACHE_PATH: cachedir,
    });
...
describe('processDriftctlOutput', () => {
  it('test that html in stdout is parsed correctly', async () => {
    ...
    const expectedOutputData = fs.readFileSync(
      path.join(getFixturePath('iac'), 'drift', 'output', 'w3security_output.html'),
    );
  ...
  it('test that html in file is parsed correctly', async () => {
    ...
    const tmpFilepath = '/tmp/w3security-html-test.html';
