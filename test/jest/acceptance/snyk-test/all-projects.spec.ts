...
describe('w3security test --all-projects (mocked server only)', () => {
  let server;
  ...
  test('`test yarn-out-of-sync` skips the out of sync project and scans the rest', async () => {
    ...
    expect(stderr).toMatch(
      `Dependency w3security@1.320.0 was not found in yarn.lock. Your package.json and yarn.lock are probably out of sync. Please run "yarn install" and try again.`,
    );
  });
  ...
  test('`test mono-repo-with-ignores --all-projects` respects .w3security policy', async () => {
    ...
    expect(backendRequests[0].headers['x-w3security-cli-version']).toBeDefined();
    ...
    expect(backendRequests[1].headers['x-w3security-cli-version']).toBeDefined();
  ...
  test('`test composer-app --all-projects`', async () => {
    ...
    backendRequests.forEach((req) => {
      ...
      expect(req.headers['x-w3security-cli-version']).not.toBeUndefined();
