...
describe('config', () => {
  ...
  it('loads API_REST_URL from config key if set', async () => {
    const env = {
      ...process.env,
      W3SECURITY_API_REST_URL: 'http://localhost:' + port + baseURL,
    };
  ...
  it('fails with incorrect base url for sanity check', async () => {
    const env = {
      ...process.env,
      W3SECURITY_API_REST_URL: 'http://localhost:' + port + '/wrongbase',
    };
  ...
  it('loads API_REST_URL from API_V3_URL if set', async () => {
    const env = {
      ...process.env,
      W3SECURITY_API_V3_URL: 'http://localhost:' + port + baseURL,
    };
