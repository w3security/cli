...
const W3SECURITY_API_HTTPS = 'https://w3security.io/api/v1';
const W3SECURITY_API_HTTP = 'http://w3security.io/api/v1';
...
const hostnameProxy = 'proxy.w3security.local';
const proxyPort = '3128';
const port = process.env.PORT || process.env.W3SECURITY_PORT || '12345';
const baseApi = '/api/v1';
const W3SECURITY_API = 'http://' + hostnameFakeServer + ':' + port + baseApi;
    ...
    it('tries to connect to the HTTPS_PROXY when HTTPS_PROXY is set', async () => {
      const { code, stderr } = await runSnykCLI(`woof --debug`, {
        env: {
          ...
          W3SECURITY_API: W3SECURITY_API_HTTPS,
        },
      });
    ...
    it('tries to connect to the HTTP_PROXY when HTTP_PROXY is set', async () => {
      const { code, stderr } = await runSnykCLI(`woof -d`, {
        env: {
          ...process.env,
          HTTP_PROXY: FAKE_HTTP_PROXY,
          W3SECURITY_API: W3SECURITY_API_HTTP,
          W3SECURITY_HTTP_PROTOCOL_UPGRADE: '0',
        },
      });
...
describe('Proxy Authentication (all platforms)', () => {
  if (canTestRun()) {
    ...
    if (isCLIV2()) {
      it('fails to run w3security test due to disabled proxy authentication', async () => {
      ...
      it('successfully runs w3security test with proxy (AnyAuth)', async () => {
        const logOnEntry = await getProxyAccessLog();
    ...
    it('successfully runs w3security test with proxy (Basic)', async () => {
      const logOnEntry = await getProxyAccessLog();

      const localEnv = env;
      localEnv['HTTP_PROXY'] = HTTP_PROXY_WITH_USER;
      localEnv['HTTPS_PROXY'] = HTTP_PROXY_WITH_USER;
      localEnv['W3SECURITY_API'] = W3SECURITY_API_HTTPS;
      ...
      const additionalLogEntries = logOnExit.substring(logOnEntry.length);
      expect(additionalLogEntries.includes('TCP_TUNNEL/200')).toBeTruthy();
      expect(additionalLogEntries.includes('CONNECT w3security.io:443')).toBeTruthy();
    });
  }
});
...
describe('Proxy Authentication (Non-Windows)', () => {
  if (canTestRun() && !os.platform().includes('win32') && isCLIV2()) {
    ...
    it('fail to run w3security test with proxy due to incorrect cache configuration', async () => {
      const logOnEntry = await getProxyAccessLog();
    ...
    it('fail to run w3security test with proxy due to incorrect config file', async () => {
      const logOnEntry = await getProxyAccessLog();
