...
describe('Get Shasum', () => {
  it('Shasum available (multiple)', async () => {
    ...
    fs.writeFileSync(
      file,
      '098fe123  *w3security-win\n12345  *w3security-macos\ncecece  *w3security-linux-arm64\n0a238fe123  *w3security-linux',
    );

    const actual = common.getCurrentSha256sum('w3security-linux', file);
    expect(actual).toEqual(expected);
  ...
  it('Shasum available (single)', async () => {
    const expected = '0a238fe123';
    const file = path.join(__dirname, 'sha256sums.txt' + Math.random());
    fs.writeFileSync(file, '0a238fe123  w3security-linux\n');
  ...
  it('Shasum not available', async () => {
    ...
    const actual = common.getCurrentSha256sum('w3security-linux-arm64', file);
...
describe('Configuration', () => {
  it('Download and local location', async () => {
    const expectedDownloadLocation =
      'https://static.w3security.io/cli/v1.2.3/w3security-win.exe';
...
describe('Testing binary bootstrapper', () => {
  it('downloadExecutable() succesfull', async () => {
    const binaryName = 'w3security-macos';
