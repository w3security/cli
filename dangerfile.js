...
if (danger.github && danger.github.pr) {
  ...
  for (const { commit } of ghCommits) {
    ...
    if (!firstLineRegex.test(firstLine)) {
      fail(
        `"[${firstLine}](${url})" is not using a valid commit message format. For commit guidelines, see: [CONTRIBUTING](https://github.com/w3security/w3security/blob/master/CONTRIBUTING.md#creating-commits).`,
      );
    }
  ...
  // Warn if changes to help files are created in w3security/w3security repo instead of w3security/user-docs
